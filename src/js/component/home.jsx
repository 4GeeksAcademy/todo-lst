...
function TodoList() {
    ...
    return (
        <div className="container text-light d-flex position-absolute top-50 start-50 translate-middle flex-column col-4 my-3 mx-auto shadow-lg p-3 mb-5 bg-black rounded-4 " >
            <h1 className="text-center mt-4">Todo List</h1>

            <input type="text" value={newTask} onChange={e => setNewTask(e.target.value)} onKeyDown={handleKeyDown} placeholder="Que deseas añadir...?" />
            <button onClick={addTask} className="justify-content-end btn btn-success mt-1">Añadir</button>
            ...
            {isTaskEmpty && <p className="text-center">No hay tareas</p>}
        </div>
    );
};
